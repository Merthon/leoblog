...
description: "整理 JavaScript 的变量声明、基础类型、运算符、字符串、数组和对象。"
...
### 声明变量
使用变量，你需要做的第一步就是创建它 -- 更准确的说，是声明一个变量。现代 JavaScript 通常优先使用 `const`；确实需要重新赋值时使用 `let`。`var` 是函数作用域，存在变量提升等历史行为，新代码一般不需要它：
```
let myName;
let myAge;
...
myName = 'Bob';
```
同一块级作用域内不能重复声明 `let` 或 `const`。`const` 变量不能再次赋值，但对象内部的属性仍可以修改。
### 变量类型
#### Number
JavaScript 的 `Number` 使用 IEEE 754 双精度浮点数表示整数和小数；超出安全整数范围时应考虑 `BigInt`。JavaScript 是动态类型语言，变量声明时不需要标注数据类型。
```
let myAge =  17;
```
##### 数字类型
